...
public class CityValidatorBean {
    ...
    public void validateCityCreateRequest(CityCreateRequestDto dto) {
        ...
        if (dto.getGovernor() == null) {
            throw new ValidationException("Governor is required");
        } else {
            if (dto.getGovernor().getName() == null || dto.getGovernor().getName().trim().isEmpty()) {
                throw new ValidationException("Governor name is required");
            }
            if (dto.getGovernor().getAge() != null && dto.getGovernor().getAge() <= 0) {
                throw new ValidationException("Governor age must be greater than 0");
            }
            if (dto.getGovernor().getHeight() != null && dto.getGovernor().getHeight() <= 0) {
                throw new ValidationException("Governor height must be greater than 0");
            }
        }
    }
}
